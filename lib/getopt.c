 ...
 * contain conflicting prototypes for getopt. On the other hand, implicit
 * declarations of getenv and friends are invalid in C99, which produces
 * warnings on Apple systems (at the least)... */
# include <stdlib.h>
# include <unistd.h>
#endif	/* GNU C library.  */
...
/* It is not Unix, really. See? CAPITAL LETTERS. */
# include <windows.h>
# define getpid() GetCurrentProcessId()
#endif /* WIN32 && !__CYGWIN32__ */
...
/* We want to avoid inclusion of string.h with non-GNU libraries
   ...
   in GCC. On the other hand, implicit declarations of strlen and friends are
   invalid in C99, which produces warnings on Apple systems... */
# include <string.h>
...
# define NONOPTION_P (argv[optind][0] != '-' || argv[optind][1] == '\0')
   ...
   *
   * On the other hand, if there is a long option "fubar" and
  ...
  if (longopts != NULL && (argv[optind][1] == '-'
      ...
      if (pfound != NULL) {
		  ...
		  if (*nameend) {
			  ...
			  if (pfound->has_arg) {
				  ...
				  } /* where to put the closing brace was ambiguous; could also
					 * have gone around the next line ending in a semicolon, as
					 * far as I could tell... */
	...
	/* What were those brackets doing dangling here? (I removed them) */
	char c = *nextchar++;
    ...
    if (temp[0] == 'W' && temp[1] == ';') {
		...
		/* optarg is now the argument, see if it is in the
		 * table of longopts.  */
